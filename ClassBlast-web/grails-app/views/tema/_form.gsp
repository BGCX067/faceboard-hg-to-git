...
<div class="fieldcontain ${hasErrors(bean: temaInstance, field: 'sectionRelated', 'error')} required">
	...
	<g:select id="sectionRelated" name="sectionRelated.id" from="${classblast.web.Seccion.list()}" optionKey="id" required="" value="${temaInstance?.sectionRelated?.id}" class="many-to-one"/>
</div>
