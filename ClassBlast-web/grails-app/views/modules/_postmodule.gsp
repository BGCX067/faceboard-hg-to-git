	...
	<div style="width: 70%; border: solid #8BC600; margin: auto">
		<section id="show_post${post.id}">
			...
			<div style="padding-right: 5px; float: left;">
				<g:if test="${isOwn}">
					<g:link
						onclick="enableEditPost(${post.id},'${post.postBody}');return(false)"
				...
				<span class="add_comment" id="add_comment${post.id}"> <g:link
		...
		<section id="edit_post${post.id}" class="post_edit"
			style="display: none">
			<g:formRemote name="ep" on404="alert('not found!)" update="post_list"
				url="[action:'editPost']">
				<div id="pu-tfield">
				...
				<div id="pu-bt">
					<g:actionSubmit value="Listo" onclick="repaintPost(${post.id})" />
		...
		<span class="comment_field" id="comment_field${post.id}"
			style="display: none"> <g:formRemote
